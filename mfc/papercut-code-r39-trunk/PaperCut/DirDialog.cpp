/*!
  ...
  $Id: DirDialog.cpp 9 2006-03-08 14:41:10Z henry_groover $
...
// Called by MakeSureDirectoryPathExists(). Assert existence of previous portion of path
static int CreatePreviousPathPart( LPCTSTR lpDir )
{
	// Get previous part
	CString szPrevious( lpDir );
	szPrevious = szPrevious.Left( szPrevious.GetLength() - 1 );
	int TailLength = szPrevious.ReverseFind( '\\' );
	if (TailLength <= 0)
		return 0;
	szPrevious = szPrevious.Left( TailLength );
	if (_access( szPrevious, 00 ) != 0)
	{
		szPrevious += '\\';
		if (!CreatePreviousPathPart( szPrevious ))
			return 0;
		szPrevious = szPrevious.Left( TailLength );
	}
	if (_mkdir( szPrevious ) != 0 && errno != EEXIST)
		return 0;
	return 1;
}
...
int AssertDirectoryPathExists( LPCTSTR lpDir )
{
	if (!_access( lpDir, 00 ))
		return 1; // Exists
	// Try to create preceding part
	if (!CreatePreviousPathPart( lpDir ))
		return 0; // Failed
	// Finally, try to create path
	if (_mkdir( lpDir ) != 0 && errno != EEXIST)
		return 0; // Failed
	return 1;
}
        ...
        if (attr == 0xFFFFFFFF)
        {
            ...
            else
            {
                ...
                if (AfxMessageBox(mess, MB_YESNO) == IDYES)
                {
                    ...
                    if (!::AssertDirectoryPathExists(ss + _T("\\")))
                    {
...
void CDlgWnd::CheckDir(const CString &ss)
{
    ...
    CEdit *pOld = (CEdit *)GetDlgItem(editctl);
    ASSERT(pOld != NULL);
...
void CDirDialog::OnInitDone()
{
	...
	CWnd* pEdit = pp->GetDlgItem(m_DlgWnd.editctl = edt1);
//	if (pEdit == NULL) 
//	{
//		pEdit = pp->GetDlgItem(m_DlgWnd.editctl = cmb2);
//	}
	if (pEdit == NULL) 
	{
		pEdit = pp->GetDlgItem(m_DlgWnd.editctl = cmb13);
	}
    ASSERT(pEdit != NULL);
    pEdit->GetWindowRect(rct); //Get edt1 rectangle
...
//  m_Edit.SetSel(0, strlen(m_ofn.lpstrInitialDir));
    ...
    pp->ScreenToClient(rct);
    pEdit->SetWindowPos(NULL, rct.left, rct.top+70,
                   0, 0, SWP_NOZORDER | SWP_NOSIZE);

#else
    // Hide the controls we don't want the user to use
    HideControl(IDOK);
    HideControl(m_DlgWnd.editctl);
#endif
